...
function getRequestData(data: unknown): Record<string, unknown> {
...
export const createStudentAccount = onCall(
  ...
  async (request) => {
    try {
      const data = getRequestData(request.data);
      ...
      const studentAccount = await getAuth().createUser({
        email,
        password,
        emailVerified: false,
        disabled: false,
...
export const updateStudentPassword = onCall(
  ...
  async (request) => {
    try {
      ...
      await getAuth().updateUser(studentId, {
