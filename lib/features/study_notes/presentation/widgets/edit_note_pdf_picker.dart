...
class EditNotePdfPicker extends StatefulWidget {
  const EditNotePdfPicker({
    super.key,
  ...
  final ValueChanged<EditNotePdfFile> onReplacementSelected;

  final VoidCallback onReplacementRemoved;

  @override
  State<EditNotePdfPicker> createState() {
    return _EditNotePdfPickerState();
  }
}

class _EditNotePdfPickerState extends State<EditNotePdfPicker> {
  ...
  Future<void> _pickPdfFile() async {
    ...
    try {
      ...
      debugPrint('Edit note PDF picker error: $error');
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedSwitcher(
      ...
      child: _isPickingFile
          ? const _EditPdfPickerLoading()
          : replacementPdf != null
          ? _EditSelectedPdfPicker(
              key: ValueKey('replacement-${replacementPdf.path}'),
          ...
          : hasExistingFile
          ? _EditSelectedPdfPicker(
              key: ValueKey('existing-$existingFileName'),
          ...
          : _EmptyEditPdfPicker(onTap: _pickPdfFile),
    );
  }
}

class _EmptyEditPdfPicker extends StatelessWidget {
  const _EmptyEditPdfPicker({required this.onTap});
  ...
  Widget build(BuildContext context) {
    return Semantics(
      key: const ValueKey('empty-edit-pdf-picker'),
      button: true,
...
class _EditSelectedPdfPicker extends StatelessWidget {
  const _EditSelectedPdfPicker({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Semantics(
      button: true,
      label: 'ملف PDF موجود، اضغط لاستبداله',
      child: Container(
        width: double.infinity,
        height: 116.h,
        decoration: BoxDecoration(
...
class _EditPdfPickerLoading extends StatelessWidget {
  const _EditPdfPickerLoading();

  @override
  Widget build(BuildContext context) {
    return Container(
      key: const ValueKey('edit-pdf-picker-loading'),
      width: double.infinity,
      height: 116.h,
      decoration: BoxDecoration(
        color: ColorPalette.surface,
